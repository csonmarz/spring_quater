...
def brighten(image, factor):
    ...
    x_pixels, y_pixels, num_chanels = image.array.shape # get x, y pixels of image, # channels 
    # make an empty image so we dont actually modify the original one
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_chanels)

    #this is the most intuitive qay to do this(non-vectorized)
    #for x in range(x_pixels):
        #for y in range(y_pixels):
            #for c in range(num_chanels):
...
def adjust_contrast(image, factor, mid):
    ...
    x_pixels, y_pixels, num_chanels = image.array.shape 
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_chanels)
...
def blur(image, kernel_size):
    ...
    neighbor_range = kernel_size  // 2 # how many enighbors to one side we look at 

    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_chanels):
            # we are going to use a vaive implementation of literating through each neighbor and summing
            # there is a faster way but this is more straightfoward to understand
                total = 0
            ...
            new_im.array[x_i, y_i, c] = total / (kernel_size ** 2)
